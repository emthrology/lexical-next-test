...
import { StyledLinkNode, $createStyledLinkNode } from './StyledLinkNode';
...
export default function LinkButtonPlugin() {
  ...
  const addLink = (text, url) => {
    editor.update(() => {
      ...
      // 스타일이 적용된 링크 노드 생성
      const styledLink = $createStyledLinkNode(
        url,
        text,
        'text-decoration: underline; color: blue;'
      );

      root.append(styledLink); // 링크를 루트에 추가
    });
  ...
  return (
    <>
      ...
      <div id="linkModal" className="link-modal" style={{ display: 'none' }}>
        <div
        ...
        >
          ...
          />
          <button
            onClick={() => addLink(linkLabel, linkUrl)}
            className="btn-01 m-t-20"
            id="submitLink"
          >
            입력
          </button>
